...
for w in range(1, imgmaskkk.size[0] - 1):
    for h in range(1, imgmaskkk.size[1] - 1):
        if imgmaskkk.getpixel((w, h)) != (0, 0, 0, 255):
            if imgmaskkk.getpixel((w + 1, h)) == (0, 0, 0, 255) or imgmaskkk.getpixel((w - 1, h)) == (0, 0, 0, 255):
                compound.putpixel((w, h), (255, 255, 255, 0))
                compound.putpixel((w + 1, h), (255, 255, 255, 0))
                compound.putpixel((w - 1, h), (255, 255, 255, 0))
            if imgmaskkk.getpixel((w, h + 1)) == (0, 0, 0, 255) or imgmaskkk.getpixel((w, h - 1)) == (0, 0, 0, 255):
                compound.putpixel((w, h), (255, 255, 255, 0))
...
cv2.imwrite(floder + blockTemp1, cropped)

deleteBlack(floder + blockTemp1)

# 给滑块加阴影
cutImage = Image.open(floder + blockTemp1)
